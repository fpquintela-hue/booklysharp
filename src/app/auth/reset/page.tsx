 ...
 * This page handles password resets without needing the tenant alias in the URL path.
 * The tenant context is retrieved from the token verification on the server.
 */
export default function GlobalResetPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/auth/reset-password', {
                ...
                // Note: alias is now optional as the server can resolve it from the token
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    const target = data.alias ? `/${data.alias}/login` : '/';
                    router.push(target);
                }, 3000);
            } else {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans">
                <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full text-center border border-slate-100">
                    ...
                    <h1 className="text-2xl font-black mb-2 uppercase tracking-tight text-slate-900">{t('reset.error')}</h1>
                    <p className="text-slate-500 mb-6 font-medium">O enlace no é válido ou xa foi utilizado.</p>
                    <button onClick={() => router.push('/')} className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-2xl transition-all">
            ...
            <div className="bg-[#F9FAFB] min-h-screen w-full flex items-center justify-center font-sans antialiased relative overflow-hidden">
                ...
                <main className="relative z-10 w-full max-w-md px-6 py-8 flex flex-col items-center justify-center">
                    <div className="text-center mb-8 flex flex-col items-center">
                        <div className="bg-white p-4 rounded-3xl shadow-xl mb-6 w-48 h-auto flex items-center justify-center border border-slate-100/50">
                    ...
                    <div className="bg-[#2563EB] w-full rounded-[2.5rem] p-10 relative overflow-hidden shadow-2xl shadow-[#2563EB]/30 border border-white/10">
                        ...
                        {!success && (
                            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                            <span className="material-symbols-outlined text-white/60 group-focus-within:text-white transition-colors">lock</span>
                                        </div>
                                        <input
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                            <span className="material-symbols-outlined text-white/60 group-focus-within:text-white transition-colors">lock_reset</span>
                                        </div>
                                        <input
                                            className="block w-full pl-11 pr-4 py-4 rounded-2xl bg-white/15 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 text-white placeholder-white/50 transition-all font-medium"
                                            type="password"
                        ...
                        {success && (
                            <button
                                onClick={() => router.push(userAlias ? `/${userAlias}/login` : '/')}
